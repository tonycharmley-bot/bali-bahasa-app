...
export default function App() {
  ...
  useEffect(() => {
    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ profiles, activeProfile, showProfileManager })
    );
  ...
  const pool = useMemo(() => {
    ...
    if (currentUser.showReviewOnly) {
      filtered = filtered.filter((d) => currentUser.wrongIds.includes(d.id));
    }
    if (currentUser.playMode === "listening") {
  ...
  useEffect(() => {
    ...
    if (!sameUnlocked || currentUser.comboMultiplier !== nextCombo) {
      updateUser({ unlocked: nextUnlocked, comboMultiplier: nextCombo });
    }
  }, [
    currentUser.score,
    ...
    currentUser.answeredIds.length,
    currentUser.hearts,
    currentUser.streak,
    currentUser.comboMultiplier,
  ...
  const startVoiceInput = () => {
    ...
    recognition.onerror = () => {
      updateUser({
        feedback: {
          ok: false,
          text: "Voice input did not work",
          explanation: "Please try again or type your answer manually."
        }
      });
    };
  ...
  const checkAnswer = (submitted) => {
    ...
    if (isCorrect) {
      ...
      updateUser({
        ...
        streak: 0,
        wrongIds: currentUser.wrongIds.includes(current.id)
          ? currentUser.wrongIds
          : [...currentUser.wrongIds, current.id],
        feedback: {
          ok: false,
          text: "Not quite",
  ...
  const installApp = async () => {
    if (!deferredPrompt) {
      updateUser({
        feedback: {
          ok: false,
          text: "Install prompt not ready",
          explanation: "In Chrome, use the browser menu and choose Add to Home screen."
        }
      });
      return;
    }
    ...
    if (choice?.outcome === "accepted") {
      ...
      updateUser({
        feedback: {
          ok: true,
          text: "App install started",
          explanation: "Open it from your home screen for the best full-screen experience."
        }
      });
    }
  };
  ...
  const createProfile = () => {
    ...
    setProfiles((prev) => ({
      ...prev,
      [clean]: prev[clean] || blankUserState()
    }));
              ...
              {Object.keys(profiles).map((name) => (
                ...
                >
                  ...
                  <div style={{ color: "#94a3b8", marginTop: 4, fontSize: 13 }}>
                    Level {getLevelData((profiles[name] || {}).score || 0).level} · {(profiles[name] || {}).score || 0} pts
                  </div>
                </button>
              ))}
            </div>
      ...
      <div style={styles.page}>
        <div style={styles.wrap}>
          <div style={styles.row}>
            ...
            <button style={buttonStyle(false, false)} onClick={() => setShowProfileManager(true)}>
              {activeProfile}
            </button>
          </div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {[5, 10, 20].map((goal) => (
                  <button
                    key={goal}
                    style={goal === currentUser.dailyGoal ? { ...styles.buttonPrimary, minHeight: 42 } : { ...styles.button, minHeight: 42 }}
                    onClick={() => updateUser({ dailyGoal: goal })}
                  >
                    {goal} drills
                  </button>
                ))}
    ...
    <div style={styles.page}>
      <div style={styles.wrap}>
        <div style={styles.card}>
          <div style={styles.row}>
            <div>
              <div style={{ color: "#86efac", letterSpacing: "0.18em", textTransform: "uppercase", fontSize: 11, fontWeight: 800 }}>
                {levelData.title}
              </div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <div style={{ ...styles.badge, background: "rgba(251,191,36,0.14)", color: "#fde68a", borderColor: "rgba(251,191,36,0.22)" }}>
          ...
          <div style={{ marginTop: 14, ...styles.progressTrack }}>
            <div style={progressFill(progressToNext)} />
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ ...styles.row, marginBottom: 6 }}>
              ...
              <div style={{ color: "#86efac", fontWeight: 700 }}>{currentUser.completedToday}/{currentUser.dailyGoal}</div>
            </div>
            <div style={styles.progressTrack}>
              <div style={progressFill((currentUser.completedToday / currentUser.dailyGoal) * 100)} />
            </div>
          </div>
        </div>
        ...
        <div style={{ ...styles.card, padding: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 8 }}>
            <button
              ...
              onClick={() => updateUser({ feedback: { ok: true, text: "Progress saved", explanation: "Your profile has been saved on this device." } })}
            >
              💾 Save
            </button>
            <button style={buttonStyle(installReady, false)} onClick={installApp}>
          ...
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, marginBottom: 8 }}>
            {categories.map((cat) => (
              <button
                key={cat}
                style={{ ...styles.button, whiteSpace: "nowrap", borderRadius: 999, ...(currentUser.mode === cat ? styles.buttonActive : {}) }}
                onClick={() => {
        ...
        {currentUser.hearts > 0 ? (
          <div style={styles.card}>
            <div style={styles.row}>
              ...
              <div style={{ ...styles.badge, background: "transparent", color: "#cbd5e1", borderColor: "rgba(255,255,255,0.15)" }}>
                Drill {currentUser.answeredIds.length + 1}
              </div>
            </div>
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    ...
                    onChange={(event) => updateUser({ input: event.target.value })}
            ...
            <div style={{ marginTop: 12, ...styles.muted }}>
              {currentUser.playMode === "typing" ? `Translate into Bahasa Indonesia. Tip: ${current.tip}` : `Tip: ${current.tip}`}
            </div>

            {currentUser.playMode === "typing" ? (
              <button style={{ ...styles.buttonPrimary, width: "100%", marginTop: 14 }} onClick={handleSubmit}>
                Check Answer
              </button>
            ) : null}
            ...
            {currentUser.feedback ? (
              <div style={{ marginTop: 14, ...(currentUser.feedback.ok ? styles.feedbackGood : styles.feedbackBad) }}>
                <div style={{ fontWeight: 800, color: currentUser.feedback.ok ? "#a7f3d0" : "#fecdd3" }}>
          ...
          <div style={styles.card}>
            ...
            <div style={{ textAlign: "center", fontSize: 26, fontWeight: 800, marginTop: 6 }}>Run Over</div>
            <p style={{ textAlign: "center", color: "#cbd5e1" }}>You ran out of lives. Reset and go again.</p>
            <button style={{ ...styles.buttonPrimary, width: "100%" }} onClick={() => updateUser(blankUserState())}>
              Start New Run
            </button>
          </div>
        )}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: unlockedNow ? "rgba(251,191,36,0.10)" : "rgba(15,23,42,0.68)",
                    borderColor: unlockedNow ? "rgba(251,191,36,0.22)" : "rgba(255,255,255,0.10)"
                  }}
                >
                  <div style={styles.row}>
                    <div>
                      <div style={{ fontWeight: 800 }}>{item.label}</div>
                      <div style={{ ...styles.muted, fontSize: 14, marginTop: 3 }}>{item.desc}</div>
                    </div>
          ...
          <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
            <div style={styles.achievementRow}>
              ...
              <div style={styles.muted}>
                {currentUser.wrongIds.length
                  ? `${currentUser.wrongIds.length} phrase${currentUser.wrongIds.length > 1 ? "s" : ""} saved for review mode.`
                  : "No saved mistakes yet. Keep going."}
              </div>
              <div style={{ ...styles.muted, fontSize: 12, marginTop: 8 }}>
                Adaptive mode prioritizes phrases you miss more often and eases off on phrases you keep getting right.
              </div>
            </div>
